...
const Women = () => {
  ...
  const availableSubCategories = useMemo(
    () => [...new Set(womenProducts.map((p) => p.subCategory))],
    [womenProducts]
  );

  const availableSizes = useMemo(() => {
    const set = new Set();
    womenProducts.forEach((p) => (p.sizes || []).forEach((s) => set.add(s)));
    ...
    <>
      <div className="women-layout">
        ...
        <div className="women-filter-group">
          <div className="women-filter-header" onClick={() => toggleSection("subCategory")}>
            <h3>Subcategory</h3>
            <span>{openSections.subCategory ? <RiArrowDownWideLine /> : <RiArrowRightWideLine />}</span>
          </div>
          {openSections.subCategory && (
            <div className="women-filter-options">
              {availableSubCategories.map((sub) => (
                <label
                  key={sub}
                  className={filters.subCategory.includes(sub) ? "women-active" : ""}
                >
                  <input
                    type="checkbox"
        ...
        <div className="women-filter-group">
          <div className="women-filter-header" onClick={() => toggleSection("price")}>
            <h3>Price</h3>
            <span>{openSections.price ? <RiArrowDownWideLine /> : <RiArrowRightWideLine />}</span>
          </div>
          {openSections.price && (
            <div className="women-filter-options women-price-options">
              <label>
                <input
                  type="radio"
                  name="women-price"
        ...
        <div className="women-filter-group">
          <div className="women-filter-header" onClick={() => toggleSection("size")}>
            <h3>Size</h3>
          ...
          {openSections.size && (
            <div className="women-filter-options">
              {availableSizes.map((size) => (
                <label
                  key={size}
                  className={filters.sizes.includes(size) ? "women-active" : ""}
                >
                  <input
                    type="checkbox"
      ...
      <main className="women-products-section">
        <div className="women-top-bar">
          <p>
            Showing {paginatedProducts.length} of {filteredProducts.length} products
          </p>
          <div className="women-sort-by">
        ...
        {(filters.subCategory.length || filters.sizes.length || filters.priceRange) && (
          <div className="women-applied-filters">
            <span className="women-filter-title">Applied Filters:</span>
            {filters.subCategory.map((sub) => (
              <div key={sub} className="women-chip">
                {sub}
                <button onClick={() => toggleFilter("subCategory", sub)}>×</button>
              </div>
            ))}
            {filters.sizes.map((size) => (
              <div key={size} className="women-chip">
                {size}
        ...
        <div className="women-product-grid">
          {paginatedProducts.length === 0 && (
            <div className="women-empty">No products match your filters.</div>
          )}
          ...
          {paginatedProducts.map((p) => (
            ...
            >
              {p.bestseller && <div className="women-bestseller-badge">Bestseller</div>}
              <img src={Array.isArray(p.image) ? p.image[0] : p.image} alt={p.name} />
              <h3>{p.name}</h3>
              <p className="women-description">{p.description}</p>
              <p className="women-price">
                {currency}
                {p.price}
                {p.oldPrice && (
                  <span className="women-old-price">{currency}{p.oldPrice}</span>
                )}
              </p>
            </Link>
          ))}
          ...
          <div className="women-pagination">
            ...
            {[...Array(totalPages)].map((_, i) => (
              <button
                key={i}
                onClick={() => setPage(i + 1)}
                className={page === i + 1 ? "women-active-page" : ""}
              >
                {i + 1}
              </button>
            ))}
